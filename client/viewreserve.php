...
<body>
                                                    ...
                                                    <div class="">
                                                        ...
                                                        <div class="formbold-mb-5 w-full formbold-px-3">
                                                            <?php
                                                                ...
                                                                INNER JOIN tblNicheLocation ON tblNiche.LocID = tblNicheLocation.LocID ORDER BY Nno ASC";
                                                                $queryNno = $conn->query($selectNno);
                                                                $nnoData = array();
                                                                while($dataNno = $queryNno->fetch(PDO::FETCH_ASSOC)){
                                                        ...
                                                        <div>
                                                            <button class="formbold-btn-next" name="next"
                                                                onclick="pay('<?php echo $profID ?>', '<?php echo $nid ?>')">Proceed</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
    ...
    function updateDetailsOnChange(select) {
        ...
        var selectedProfID = document.getElementById('profID').value; // Corrected variable name
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                locIdInput.value = selectedLocId;
                nidInput.value = selectedNid;
                updateDetails(selectedNno, selectedLocId, selectedNid, selectedAppID, selectedProfID);
            } else {
    ...
    function updateDetails(selectedNno, selectedLocId, selectedNid, selectedAppID, selectedprofID) {
